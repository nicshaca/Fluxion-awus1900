...
if [ ! $FLUXIONDebug ]; then
  ...
  sleep 0.1
  format_center_literals "${CSRed}FLUXION $FLUXIONVersion$CClr (rev. $CSBlu$FLUXIONRevision$CClr)$CYel edited by$CWht nicshaca"
  echo -e "$FormatCenterLiterals"
...
function fluxion_unset_interface() {
  ...
  if [ ${#wiMonitors[@]} -gt 0 ]; then
    ...
    for monitor in ${wiMonitors[@]}; do
      ...
      iw dev "wlan0v" del &> $FLUXIONOutputDevice
      iw dev "wlan1v" del &> $FLUXIONOutputDevice

      # Remove monitoring interface after AP interface.
      if [[ "$monitor" == *"v"  ]]; then airmon-ng stop "$monitor" >$FLUXIONOutputDevice
...
function fluxion_set_interface() {
  ...
  #  xterm -e 'sudo ifconfig '$wiSelected' down && sudo iwconfig '$wiSelected' mode monitor && sudo ifconfig '$wiSelected' up' & 
...
function fluxion_set_scanner() {
 ...
 #   local choices=("$FLUXIONScannerChannelOptionAll (2.4GHz)" "$FLUXIONScannerChannelOptionAll (5GHz)" "$FLUXIONScannerChannelOptionAll (2.4GHz & 5Ghz)"
 #"$FLUXIONScannerChannelOptionSpecific" "$FLUXIONGeneralBackOption")
    local choices=("$FLUXIONScannerChannelOptionAll (2.4GHz)" "$FLUXIONScannerChannelOptionSpecific" "$FLUXIONGeneralBackOption")
...
function fluxion_set_ap_service() {
  ...
  #  local choices=("$FLUXIONAPServiceHostapdOption" "$FLUXIONAPServiceAirbaseOption" "$FLUXIONGeneralBackOption")
    local choices=("$FLUXIONAPServiceHostapdOption"  "$FLUXIONGeneralBackOption")
